(() => {
  ...
  async function processBackfillPhoto(photo) { const response=await fetch(photo.url,{cache:"no-store",referrerPolicy:"same-origin"}); if(!response.ok)throw new Error(`Download failed (${response.status})`); const original=await response.blob(); const [hash,thumbnail]=await Promise.all([sha256(original),makeThumbnail(original)]); const form=new FormData(); form.append("photoId",photo.id); form.append("sha256",hash); form.append("thumbnail",thumbnail,`${photo.id}.${thumbnail.type==="image/webp"?"webp":"jpg"}`); return api("/api/index-buschgirl",{method:"POST",body:form}); }
